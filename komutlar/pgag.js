...
module.exports.run = async (bot, message, args) => {
  ...
  if(!mutekisi) return message.reply(`:warning: Lütfen bir kullanıcı etiketleyiniz! \nDoğru Kullanım; \`${prefix}pgag <@kullanıcı> <1sn/1dk/1sa/1g>\``).then(message => message.delete(5000));  
  if(mutekisi.hasPermission("ADMINISTRATOR")) return message.reply(`:warning: Yetkili bir kişiyi gaglayamam! \nDoğru Kullanım; \`${prefix}pgag <@kullanıcı> <1sn/1dk/1sa/1g>\``).then(message => message.delete(5000)); 
  message.delete()
...
message.delete()
  ...
  setTimeout(function(){
    ...
    message.channel.send(`<@${mutekisi.id}> kullanıcısının gagı sona erdi!`).then(message => message.delete(5000));  
  }, ms(mutezaman));
}
...
exports.conf = {
  ...
  exports.help = {
    ...
    usage: "pgag <@kullanıcı> <1sn/1dk/1sa/1g>"
  };